 ...
 const [showLeaveModal, setShowLeaveModal] = useState(false); 
  ...
  const fetchBoardData = async () => {
    try {
      ...
      // Set 'owner' role if ID matches absolute creator
      if (ownerId === myId) {
          setUserRole('owner'); 
      } else {
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white font-sans overflow-hidden relative transition-colors duration-500">
      ...
      <div className="flex-1 p-6 md:p-10 overflow-x-auto overflow-y-hidden relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
           ...
           <div className="flex items-center gap-4">
              ...
              {/* Only Owner and Admin can see the creation bar */}
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-8 h-[calc(100vh-16rem)] pb-6 min-w-max">
            {columnOrder.map(id => (
              <BoardColumn 
                key={id} 
                column={{ ...columns[id], taskIds: getFilteredTaskIds(id) }} 
                tasks={tasks} 
                searchQuery={searchQuery} 
                priorityFilter={priorityFilter} 
                userRole={userRole} 
                onEdit={setEditingTask} 
                onDelete={(userRole === 'owner' || userRole === 'admin') ? setDeleteData : null}
              />
            ))}
          </div>
        </DragDropContext>
      ...
      {editingTask && <EditTaskModal task={editingTask} members={members} userRole={userRole} onClose={() => setEditingTask(null)} onSave={handleUpdateTask} />}
