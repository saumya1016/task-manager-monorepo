...
      setBoardData(board); // Save board details
...
        onLeave={() => setShowLeaveModal(true)} 
        /* Logic to identify absolute creator */
        isOwner={currentUser?._id === boardData?.owner?._id || currentUser?._id === boardData?.owner}
      />
...
                            className={`w-9 h-9 rounded-full border-2 flex items-center justify-center overflow-hidden transition-all relative hover:z-10 hover:scale-110
                                ${filterUserId === member._id ? 'border-indigo-500 z-20 bg-indigo-600' : 'border-white dark:border-zinc-950 bg-zinc-100 dark:bg-zinc-800'}`}
                        >
                            {member.profilePicture ? (
                              <img src={member.profilePicture} alt={member.name} className="w-full h-full object-cover" />
                            ) : (
                              <span className={`text-[10px] font-black uppercase ${filterUserId === member._id ? 'text-white' : 'text-zinc-500'}`}>
...
                    className={`flex items-center gap-2 text-[11px] font-black uppercase tracking-widest px-3 py-2 rounded-xl transition-all ${filterUserId === currentUser?._id ? 'text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10' : 'text-zinc-400 hover:text-zinc-900'}`}
                 >
                    {currentUser?.profilePicture && (
                      <img src={currentUser.profilePicture} alt="me" className="w-5 h-5 rounded-full object-cover border border-zinc-200 dark:border-zinc-700" />
                    )}
                    My Tasks
                 </button>
                 
...
              {/* Creation Lock: Hide creation bar from viewers */}
              {userRole !== 'viewer' && (
                 <NewTaskBar 
...
            {columnOrder.map(id => (
              <BoardColumn key={id} column={{ ...columns[id], taskIds: getFilteredTaskIds(id) }} tasks={tasks} searchQuery={searchQuery} priorityFilter={priorityFilter} isViewer={userRole === 'viewer'} userRole={userRole} onEdit={setEditingTask} onDelete={setDeleteData} />
            ))}
          </div>
        </DragDropContext>
      </div>
