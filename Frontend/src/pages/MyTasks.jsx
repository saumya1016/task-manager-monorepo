...
import { CheckCircle, ArrowRight, Layout, Search, Loader2, RefreshCw } from 'lucide-react';
...
const MyTasks = () => {
  ...
  const fetchMyTasks = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to sync workspace");
    } finally { 
      setLoading(false); 
      setRefreshing(false);
    }
  ...
  const toggleStatus = async (taskId, currentStatus) => {
    // Determine new status based on your backend enum (usually uppercase)
    ...
    try {
      ...
      fetchMyTasks(); // Re-fetch from server to correct the UI
    }
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 flex flex-col items-center py-12 px-6 font-sans">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <button 
                ...
                className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-indigo-500 transition-colors"
              >
                Personal Focus {refreshing && <RefreshCw size={10} className="animate-spin" />}
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full sm:w-64 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none text-xs w-full text-zinc-900 dark:text-white"
                value={searchQuery}
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden">
          {filteredTasks.length === 0 ? (
            <div className="p-24 text-center">
              ...
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">No tasks found.</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-tighter text-zinc-400">
                        <Layout size={10}/> {task.boardId?.title || 'External Board'}
                      </div>
                  ...
                  <div className="flex items-center gap-8">
                    <div className="hidden md:flex flex-col items-end opacity-60">
                      ...
                      <span className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">
                        {task.dueDate ? new Date(task.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'Open'}
                      </span>
                    </div>
                    <button 
                      onClick={() => navigate(`/board/${task.boardId?._id || task.boardId}`)} 
